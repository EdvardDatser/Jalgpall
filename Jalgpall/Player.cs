...
namespace Jalgpall
{
    public class Player
    {
        //Это все поля, артибуты
        public string Name { get; } // Имя игрока ,Публичная переменная "Name" метод get который возвращает значение свойства или элемент индексатора.
        ...
        private double _vx, _vy; //Расстояние мачя и игрока
        ...
        private const double MaxSpeed = 5; // Максимальная скорость мяча игрока
        private const double MaxKickSpeed = 25; // Максимальная скорость удара мачя
        private const double BallKickDistance = 10; // Дистанция на которую возможно ударить мяч

        private Random _random = new Random(); //Создается рандомное число
        //Конструкторы
        public Player(string name) // Конструктор, который запрашивает текстовое значение и присваевает к полю "Name", к строке 12.
        {
            Name = name;
        }
